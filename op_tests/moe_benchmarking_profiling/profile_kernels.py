...
Input:
    CSV file with best kernel selections from 02_analyze_results.py
...
Usage:
    python 03_profile_kernels.py -i best_kernels.csv -o profiling_output

Key Features:
    - Multi-round profiling to collect comprehensive counter data
    - Uses tuner.generate_data() for consistent data generation
    - Generates execution scripts dynamically for each kernel
    - Preserves all column names and data transformations from input
"""
...
def create_kernel_execution_script(row, script_path):
    ...
    CRITICAL: Preserves all column name handling and dtype string transformations.
    """
    ...
    # CRITICAL: dtype string handling must stay exactly as-is
...
def main():
    ...
    moe_utils.print_section_header("MULTI-ROUND PROFILING MODE")
